...
export function CreditDisplay({ className, compact = false, refreshTrigger = 0 }: CreditDisplayProps) {
  ...
  const [userKeyPoolCount, setUserKeyPoolCount] = useState(0);
  ...
  const checkUserKeys = async () => {
    ...
    try {
      // Check user_secrets for single stored key (legacy)
      ...
      // Check user_api_keys table for key pool
      ...
      setHasUserKey(!!secretData);
      setUserKeyPoolCount(count || 0);
    } catch (error) {
  ...
  // If user is admin, show unlimited
  if (status?.is_admin) {
    return (
  ...
  // If user has their own key pool, show that
  if (userKeyPoolCount > 0) {
    return (
      <div className={cn("flex items-center gap-2", className)}>
        <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/30">
          <Key className="w-3 h-3 mr-1" />
          Your API Keys ({userKeyPoolCount})
        </Badge>
      </div>
    );
  }

  // If user has single stored key (legacy)
  if (hasUserKey) {
    return (
      <div className={cn("flex items-center gap-2", className)}>
        <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/30">
