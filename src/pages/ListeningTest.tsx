...
import { Filter, X, ArrowLeft, ArrowRight, StickyNote, WifiOff, Volume2 } from 'lucide-react';
import {
  ResizablePanel,
  ResizablePanelGroup,
...
export default function ListeningTest() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!user) {
        // Use shared utility for pending state
        savePendingTestState({
          testId: testId!,
          testType: 'listening',
          answers,
      ...
      // Build question results using shared utilities
      const questionResults = questions.map(q => {
        ...
        if (dbCorrectAnswer) {
          if (q.question_type === 'MULTIPLE_CHOICE_MULTIPLE') {
            isCorrect = checkMCMAAnswer(userAnswer, dbCorrectAnswer);
          } else {
            isCorrect = checkAnswerWithAlternatives(userAnswer, dbCorrectAnswer);
          }
        }
      ...
      // Use official IELTS raw score table for full 40-question tests
      // Use percentage-based for filtered/part tests
      const isFullTest = total === 40 && !filterType && !filterPart;
      const bandScore = isFullTest 
        ? calculateBandScoreFromRaw40(score) 
        : calculateBandScoreFromPercentage(percentage);
      ...
      // Save to database with retry for network errors
      let retryToastId: string | undefined;
      
      const { submission, submissionId: finalId } = await withRetry(
        async () => {
      ...
      if (submission) {
        submissionId = finalId as `${string}-${string}-${string}-${string}-${string}`;
      }
      
      // Use shared utility to clear pending state
      clearPendingTestState();
  ...
  if (submissionError) {
    return (
      <SubmissionErrorState
        ...
        testTopic={test?.title}
        module="listening"
        backLink="/listening"
        backLabel="Return to Listening Tests"
      />
    );
  }
          ...
          {(test.audio_url || test.audio_url_part1 || test.audio_url_part2 || test.audio_url_part3 || test.audio_url_part4) && (
            <div className="hidden md:block flex-1 max-w-xl mx-4">
              <WebAudioScheduledPlayerSafe 
                ...
                onFallbackUsed={() => setUsingDeviceAudio(true)}
              />
            </div>
          )}
